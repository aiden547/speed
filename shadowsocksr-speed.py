...
test_option={}
test_option ['ping']=test_option ['network']=test_option ['speed']= test_option ['abc']= test_option ['icbc']= test_option ['cmb']= test_option ['ccb']= test_option ['ceb']= test_option ['boc']= test_option ['cmbc']=False
max_cols=0
    ...
    while True:
        ...
        if key in [curses.KEY_UP, ord('w'),
            ...
            if key == 10:
                ...
                if Option==0:
                    test_option['abc'] = test_option['ping'] = test_option['speed'] = test_option['network'] = test_option['icbc'] = test_option['cmb'] = test_option['ccb'] = test_option['ceb'] = test_option['boc'] = test_option['cmbc'] =True
                break
...
def connect_ssr(ssr):
    ...
    try:
        ...
        if test_option['abc']:
            ...
            if test_option['ccb']:
                start = time.time()
                r = requests.get('http://www.ccb.com/cn/home/indexv3.html', timeout=15)
                if r.status_code == 200:
                    end = time.time()
                    print("ccb_test:", (end - start) * 1000)
                    t = end - start
                    result['ccb'] = int(round(t * 1000))
                    result['state'] = "Success"
                else:
                    result['ccb'] = 0
                    result['state'] = "Fail"
            ...
            if test_option['boc']:
                start = time.time()
                r = requests.get('http://www.boc.cn', timeout=15)
                if r.status_code == 200:
                    end = time.time()
                    print("boc_test:", (end - start) * 1000)
                    t = end - start
                    result['boc'] = int(round(t * 1000))
                    result['state'] = "Success"
                else:
                    result['boc'] = 0
                    result['state'] = "Fail"
            ...
            if test_option['cmbc']:
                start = time.time()
                r = requests.get('http://www.cmbc.com.cn/', timeout=15)
                if r.status_code == 200:
                    end = time.time()
                    print("cmbc_test:", (end - start) * 1000)
                    t = end - start
                    result['cmbc'] = int(round(t * 1000))
                    result['state'] = "Success"
                else:
                    result['cmbc'] = 0
                    result['state'] = "Fail"
